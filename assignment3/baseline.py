...
class BaselineRuBERT(pl.LightningModule):
    ...
    def compute_iobes_score(self, sorted_zip, mode="test"):
        ...
        for (
            ...
            tag_spans = [
                (t_idx, self.id_to_tag[int(pred)]) for t_idx, pred in enumerate(preds)
            ]
            pred_entities = []
            ...
            all_entities.append(
                (cid, context, filename, txtdata, tid, c_start, c_end, pred_cl_entities)
            )
        ...
        for filename in filenames:
            ...
            for context_entities in file_entities:
                ...
                for cl_start, cl_end, tag in pred_cl_entities:
                    ...
                    text_entities.append(
                        (int(cl_start + c_start), int(cl_end + c_start), tag)
                    )

                    try:
                        assert (
                            context[cl_start : cl_end + 1]
                            == txtdata[cl_start + c_start : cl_end + 1 + c_start]
                        )
            ...
            lines.append(
                json.dumps({"id": tid, "ners": text_entities}, ensure_ascii=False)
                + "\n"
            )
...
def main():
    ...
    train_dataset = IOBESFlatRuNNEDataset(
        dataset_name="train",
    ...
    dev_dataset = IOBESFlatRuNNEDataset(
        dataset_name="dev",
    ...
    )  # Запуск процесса обучения и валидации, с мониторингом
