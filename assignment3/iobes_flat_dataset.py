...
class IOBESFlatRuNNEDataset(Dataset):
    ...
    item =
    [
        tokens : torch.LongTensor of shape (sequence_length) -
            токены последовательности после токенизации
        type_ids : torch.LongTensor of shape (sequence_length)
        labels_ids : torch.LongTensor of shape (sequence_length) -
            id сущностей в последовательности
        offsets : torch.LongTensor of shape (sequence_length, 2) -
            (start, end) диапазоны токенов в исходной последовательности
        id : torch.LongTensor of shape (1) - id контекста (предложения)
        context : str - предложение
        filename : str - имя файла, откуда взято предложение
        txtdata : str - весь текст
