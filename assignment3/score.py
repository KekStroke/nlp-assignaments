...
FEW_SHOT_TYPES = ["DISEASE", "PENALTY", "WORK_OF_ART"]


def load_jsonl(
    in_path, subdir, mode
):  # Было добавлено лишь возможность сохранять и читать другие файлы помимо test.jsonl
    data = []
    fname = os.path.join(in_path, subdir, mode + ".jsonl")
...
class Evaluator:
    def __init__(self, in_path, mode):
        self.eval_data = load_jsonl(in_path, "ref", mode)
        self.pred_data = load_jsonl(in_path, "res", mode)
        ...
        self.few_shot_type_inds = [self.ner_maps[tname] - 1 for tname in FEW_SHOT_TYPES]
    ...
    def validate(self):
        for pred_example in self.pred_data:
            for i, (s, e, t) in enumerate(pred_example["ners"], 1):
                if t not in self.ner_maps:
                    raise ValueError(
                        "{t} is unknown type name from line {i}".format(t=t, i=i)
                    )
        if len(self.eval_data) != len(self.pred_data):
            raise ValueError(
                "number of lines in ground truth is not equal to number of lines in solution passed: "
        ...
        if eval_data_ids != pred_data_ids:
            raise ValueError(
                "ids don't match. Difference: {}".format(
                    eval_data_ids.symmetric_difference(pred_data_ids)
                )
            )
    ...
    def evaluate(self):
        ...
        for gold_example in self.eval_data:
            ...
            gold_ners = set(
                [(s, e, self.ner_maps[t]) for s, e, t in gold_example["ners"]]
            )
            pred_ners = set(
                [(s, e, self.ner_maps[t]) for s, e, t in pred_example["ners"]]
            )
        ...
        m_r = 0.0 if tp == 0 else float(tp) / (tp + fn)
        m_p = 0.0 if tp == 0 else float(tp) / (tp + fp)
        m_f1 = 0.0 if m_p == 0 else 2.0 * m_r * m_p / (m_r + m_p)
        ...
        for i in range(self.num_types):
            sub_r = (
                0.0 if sub_tp[i] == 0 else float(sub_tp[i]) / (sub_tp[i] + sub_fn[i])
            )
            sub_p = (
                0.0 if sub_tp[i] == 0 else float(sub_tp[i]) / (sub_tp[i] + sub_fp[i])
            )
            sub_f1 = 0.0 if sub_p == 0 else 2.0 * sub_r * sub_p / (sub_r + sub_p)
        ...
        filtered_f1s = [
            ...
            if i not in self.few_shot_type_inds and count > 0
        ]
        summary_dict["Macro F1"] = (
            sum(filtered_f1s) / float(len(filtered_f1s))
            if len(filtered_f1s) > 0
            else 0.0
        )
        summary_dict["Macro F1 few-shot"] = sum(
            [
                each
                for i, each in enumerate(f1_scores_list)
                if i in self.few_shot_type_inds
            ]
        ) / float(len(self.few_shot_type_inds))
        print("Macro F1: {:.2f}%".format(summary_dict["Macro F1"] * 100))
        print(
            "Macro F1 few-shot: {:.2f}%".format(summary_dict["Macro F1 few-shot"] * 100)
        )
        return summary_dict, summary_dict["Macro F1"], summary_dict["Macro F1 few-shot"]
